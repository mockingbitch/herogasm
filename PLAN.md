...
>
> ✅ **HOÀN THÀNH** — xem `docs/PHASE5.md`. Giao: StoryManager (campaign state machine + story-unlock gate),
> EventManager (lifecycle Scheduled→Prep→Active→Reward→Cooldown, modifier reversible, reward-once, save),
> SeasonManager + MetaRotationService (anti-power-creep validator, buff rune/equip/synergy inject qua team_ctx)
> + BattlePassService + SeasonalShop/Currency + RankResetService + WorldEvolutionService, campaign Prologue+Ch1-3
> full + Ch4-10/arc placeholder, "Season of Frost" đầy đủ, save v6 (+migration), Telemetry+Debug, SeasonPanel UI,
> 393/393 test xanh. Ghi chú: CutsceneDef gộp vào DialogueDef; reward router chung PlayerProfile.grant_reward.
